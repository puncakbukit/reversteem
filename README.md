...
| `engine.js` | Pure game logic — board replay, ELO calculation, utility functions. No Vue, no DOM, no blockchain dependencies |
| `blockchain.js` | All Steem API interactions — RPC fallback, account fetching, reply traversal, Keychain posting |
| `components.js` | Reusable Vue 3 components — Board, PlayerBar, SpectatorConsole, MiniBoard, GamePreview, Auth controls |
...
`username` and `hasKeychain` are passed into each route view as props via `<router-view>`.
...
|---|---|
| `ProfileHeaderComponent` | Displays logged-in user's cover image, avatar, display name, and ELO rating |
| `AuthControlsComponent` | Login / logout buttons, time preset selector, timeout input, Start Game button |
| `BoardComponent` | Renders the 8×8 game board reactively; emits `cell-click`; shows overlay while a move is submitting |
| `PlayerBarComponent` | Displays both players' avatars and ELO ratings; highlights the active player with a gold glow |
| `MiniBoardComponent` | Compact 20px-per-cell board used in game previews |
...
Users may also enter a custom value (minimum 5 minutes, maximum 10,080 minutes).
...
* The board UI shows an overlay and disables clicks while a move is submitting (`isSubmitting` flag in `GameView`)
