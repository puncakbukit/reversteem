...
function updateTurnIndicator(state) {
  ...
  if (state.finished) {
    turnIndicatorDiv.innerHTML = "🏁 Game Over";
    return;
  }
  ...
  if (!playerToMove) {
    turnIndicatorDiv.innerHTML = "Waiting for opponent...";
    return;
  }

  if (username === playerToMove) {
    turnIndicatorDiv.innerHTML = `🟢 Your turn (${colorLabel})`;
  } else {
    turnIndicatorDiv.innerHTML =
...
async function handleCellClick(index) {
  ...
  const expected =
    state.currentPlayer === "black" ? state.blackPlayer : state.whitePlayer;
...
function drawMiniBoard(boardState, container) {
  ...
  for (let i = 0; i < 64; i++) {
    ...
    if (boardState[i]) {
      ...
      disk.style.background = boardState[i] === "black" ? "black" : "white";
      cell.appendChild(disk);
    }
...
} //============================================================// 6. GAME STATE DERIVATION (BLOCKCHAIN REPLAY) //============================================================function deriveGameStateFull(rootPost, replies) { let _blackPlayer=null; let _whitePlayer=null; let gameStartTime=null; let moves=[]; let timeoutMinutes=DEFAULT_TIMEOUT_MINUTES; let timeoutClaims=[]; // ---- Extract black from root ---- try { const rootMeta=JSON.parse(rootPost.json_metadata); _blackPlayer=rootMeta.black; let tm=parseInt(rootMeta.timeoutMinutes); if (isNaN(tm)) tm=DEFAULT_TIMEOUT_MINUTES; timeoutMinutes=Math.max( MIN_TIMEOUT_MINUTES, Math.min(tm, MAX_TIMEOUT_MINUTES) ); } catch {} // ---- Sort replies chronologically ---- replies.sort((a, b)=> new Date(a.created) - new Date(b.created));
...
replies.forEach(reply => {
  try {
    ...
    if (
      ...
      meta.index >= 0 &&
      meta.index < 64 && typeof meta.moveNumber === "number") {
      moves.push({
        index: meta.index,
        author: reply.author,
        moveNumber: meta.moveNumber,
...
}); // ---- Replay deterministically ---- const board=Array(64).fill(null); board[27]="white" ; board[28]="black" ; board[35]="black" ; board[36]="white" ; let appliedMoves=0; let turn="black" ; let lastMoveTime=rootPost.created; for (const move of moves) { if (move.moveNumber !==appliedMoves) continue; // pass logic if (!hasAnyValidMove(board, turn)) { const opponent=(turn==="black" ) ? "white" : "black" ; if (hasAnyValidMove(board, opponent)) { turn=opponent; } else { break; // 🔥 GAME ENDS HERE } } const expectedAuthor=turn==="black" ? _blackPlayer : _whitePlayer; if (move.author !==expectedAuthor) continue; const flips=getFlipsForBoard(board, move.index, turn); if (flips.length===0) continue; board[move.index]=turn; flips.forEach(f=> board[f] = turn);
...
function deriveGameState(rootPost, replies) {
  ...
  const latestCreated =
    replies.length > 0 ?
    replies[replies.length - 1].created :
    null;

  // ---- Validate Cache ----
  if (
    cache &&
    cache.lastCreated === latestCreated &&
...
function renderSpectatorConsole(allReplies, replies) {
  ...
  if (!replies || replies.length === 0) {
    spectatorMessagesDiv.innerHTML = `<div style="color:#555;">No spectator comments yet.</div>`;
    return;
  }
  ...
  replies
    ...
    .forEach(reply => {
      ...
      spectatorMessagesDiv.appendChild(line);
    });

  spectatorMessagesDiv.scrollTop = spectatorMessagesDiv.scrollHeight;
}
...
} //============================================================// 8. BLOCKCHAIN INTERACTIONS //============================================================async function loadOpenGames() { callWithFallback( steem.api.getDiscussionsByCreated, [{ tag: APP_NAME, limit: 20 }], async (err, posts)=> {
if (err) return;
  ...
  profileHeaderDiv.innerHTML = `
    ...
    "></div>
              ...
              <div style="display:flex; align-items:center; margin-top:-40px; padding:10px;">
                <img src="${safeUrl(data.profileImage)}" style="width:80px; height:80px; border-radius:50%; border:3px solid white; background:white;">
...
function makeMove(index) {
  ...
  if (state.currentPlayer === "white" && !state.whitePlayer) {
...
function startGame() {
  ...
  initialBoard[27] = "white";
  initialBoard[28] = "black";
  initialBoard[35] = "black";
  initialBoard[36] = "white";

  const body =
    `## New Reversteem Game\n\n` +
    `Black: @${username}\n` +
    `Timeout per move: ${timeoutMinutes} minutes\n\n` +
    boardToMarkdown(initialBoard) +
...
function isTimeoutClaimable(state) {
  ...
  const now = new Date();
  const last = new Date(lastMoveTime);
