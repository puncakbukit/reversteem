...
const App = {
  ...
  setup() {
    ...
    function login() {
      if (!window.steem_keychain) { alert("Steem Keychain extension is not installed!"); return; }
      ...
      steem_keychain.requestSignBuffer(user, "Login to Reversteem", "Posting", (res) => {
        ...
        if (verified !== user) { alert("Signed account does not match entered username."); return; }
        username.value = user;
