...
const DashboardView = {
  ...
  methods: {
    ...
    async joinGame(game) {
      if (!this.hasKeychain) {
        this.$emit("notify", { message: "Steem Keychain extension is not installed.", type: "error" });
        return;
      }
      if (!this.username) {
        this.$emit("notify", { message: "Please log in first.", type: "error" });
        return;
      }
      if (this.username === game.blackPlayer) {
        this.$emit("notify", { message: "You cannot join your own game.", type: "error" });
        return;
      }
      ...
      keychainPost(
        ...
        (res) => {
          if (!res.success) {
            this.$emit("notify", { message: "Keychain rejected the join request.", type: "error" });
            return;
          }
          this.$router.push(`/game/${game.author}/${game.permlink}`);
        }
...
const App = {
  ...
  template: `
    ...
    <router-view
      :username="username"
      :has-keychain="hasKeychain"
      @notify="n => notify(n.message, n.type)"
    ></router-view>
  `
};
