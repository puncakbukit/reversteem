...
const TIME_PRESETS = { blitz: 5, rapid: 15, standard: 60, daily: 1440 };
...
function buildGameTags(timeoutMinutes, username) {
  ...
  // Second tag: elo-[rating], rounded to nearest 100 for privacy/grouping
  const elo = getUserRating(username);
  const eloTag = `elo-${Math.round(elo / 100) * 100}`;

  return JSON.stringify([timeTag, eloTag, "reversi", "othello", "board", "game", "steem"]);
}
