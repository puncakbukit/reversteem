...
const AuthControlsComponent = {
  ...
  watch: {
    ...
    defaultTitle: {
      immediate: true,
      handler(val) {
        if (!this._titleEdited) this.gameTitle = val || "";
      }
    }
  },
  methods: {
    ...
    onTitleInput(e) {
      ...
      this._titleEdited = this.gameTitle !== this.defaultTitle;
    },
    resetTitle() {
      this._titleEdited = false;
...
const GamePreviewComponent = {
  ...
  props: {
    game: Object,
    username: String,
    profileUser: { type: String, default: null }, // set on profile page to show role badge
  ...
  template: `
    <div :style="{ marginBottom: isFeatured ? '20px' : '10px', maxWidth: '600px', margin: '0 auto 20px auto' }">
