...
const PlayerBarComponent = {
  ...
  data() {
    return { blackData: null, whiteData: null };
  },
  watch: {
    blackPlayer: { immediate: true, async handler(v) { this.blackData = v ? await fetchAccount(v) : null; } },
    whitePlayer: { immediate: true, async handler(v) { this.whiteData = v ? await fetchAccount(v) : null; } }
  },
  methods: {
    safeUrl,
    getUserRating,
    cardStyle(color) {
      ...
      const isActive = state && !state.finished && state.currentPlayer === color;
    ...
    <div :style="{ marginBottom: isFeatured ? '20px' : '10px' }">
      ...
      <div v-if="isFeatured && previewState">
        <player-bar-component
          :black-player="previewState.blackPlayer"
          :white-player="previewState.whitePlayer"
          :game-state="previewState"
        ></player-bar-component>
